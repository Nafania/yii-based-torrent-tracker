...
class NestedSetBehavior extends CActiveRecordBehavior {
	...
	public function descendants ( $depth = null ) {
		...
		$criteria->mergeWith(array(
		                          'condition'=> $alias . '.' . $db->quoteColumnName($this->leftAttribute) . '>' . $owner->{$this->leftAttribute} . ' AND ' . $alias . '.' . $db->quoteColumnName($this->rightAttribute) . '<' . $owner->{$this->rightAttribute},
	...
	public function delete () {
		...
		try {
			if ( $owner->isLeaf() ) {
				...
				$condition = $db->quoteColumnName($this->leftAttribute) . '>=' . $owner->{$this->leftAttribute} . ' AND ' . $db->quoteColumnName($this->rightAttribute) . '<=' . $owner->{$this->rightAttribute};
	...
	public function moveAsRoot () {
		...
		try {
			...
			$owner->updateAll(array(
				...
				$db->quoteColumnName($this->leftAttribute) . '>=' . $left . ' AND ' . $db->quoteColumnName($this->rightAttribute) . '<=' . $right . ' AND ' . $db->quoteColumnName($this->rootAttribute) . '=' . CDbCriteria::PARAM_PREFIX . CDbCriteria::$paramCount,
	...
	private function correctCachedOnDelete () {
		...
		if ( !empty(self::$_cached[get_class($owner)]) ) {
