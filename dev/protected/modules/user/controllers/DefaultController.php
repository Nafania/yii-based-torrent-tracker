...
class DefaultController extends components\Controller
{
    ...
    public function actionSettings()
    {
        ...
        foreach ( $services AS $service ) {
            $socialServices[$service->id] = $service;
        }
    ...
    public function actionSocialLogin($service)
    {
        ...
        /** @var EAuthServiceBase $authIdentity  */
        $authIdentity = Yii::app()->eauth->getIdentity($service);
        $authIdentity->redirectUrl = $this->createAbsoluteUrl('/user/default/settings');
        ...
        if ($authenticated) {
            ...
            /**
             * Если пользователь аутентифицировался успешно
             */
            if ($identity->authenticate()) {
                /**
                 * Проверим, новый ли это социальный аккаунт?
                 */
                ...
                if (!$UserSocialAccount) {
                    ...
                    /**
                     * Если соц. сеть отдала мыло, то поищем юзера с этим мылом
                     * Если он есть, то залогинем его
                     */
