...
class ETaggableBehavior extends CActiveRecordBehavior {
	 ...
	 */
	public $tagBindingTable = 'tagRelations';
	/**
	...
	public $tagBindingTableModelName = 'modelName';
	/**
	...
	public function getTagsWithModelsCount ( $criteria = null ) {
		$md5 = md5(serialize($criteria));

		if ( !($tags = $this->cache->get($this->getCacheKey() . 'WithModelsCount' . $md5)) ) {
			...
			$tags = $builder->createFindCommand($this->tagTable,
				$findCriteria)->queryAll();

			$this->cache->set($this->getCacheKey() . 'WithModelsCount' . $md5, $tags);
		}
	...
	private function needToSave () {
		if ( is_array($this->tags) && is_array($this->originalTags) ) {
			$diff = array_merge(array_diff($this->tags, $this->originalTags),
				array_diff($this->originalTags, $this->tags));

			return !empty($diff);
		}
		elseif ( is_array($this->tags) && $this->originalTags === null ) {
			return true;
		}
		elseif ( $this->tags === null && is_array($this->originalTags) ) {
			return true;
		}
		return false;
	}
		...
		if ( $this->needToSave() ) {
			...
			if ( !$this->createTagsAutomatically ) {
				...
				foreach ( $this->tags as $tag ) {
					...
					$tagId = $builder->createFindCommand($this->tagTable,
						$findCriteria)->queryScalar();
			...
			if ( !empty($this->tags) ) {
				foreach ( $this->tags as $tag ) {
					...
					$builder->createInsertCommand($this->getTagBindingTableName(),
						array(
	...
	protected function loadTags ( $criteria = null ) {
		...
		$tags = $this->cache->get($this->getCacheKey());

		if ( $tags === false ) {
			$findCriteria = new CDbCriteria(array(
			                                     'select'    => "t.{$this->tagTableName} as `name`",
			                                     'join'      => "INNER JOIN {$this->getTagBindingTableName()} et ON t.{$this->tagTablePk} = et.{$this->tagBindingTableTagId} AND et.{$this->tagBindingTableModelName} = :modelName ",
			...
			$tags = $this->getConnection()->getCommandBuilder()->createFindCommand($this->tagTable,
				$findCriteria)->queryColumn();
	...
	private function getCacheKey () {
		return $this->getCacheKeyBase() . $this->getOwner()->getPrimaryKey();
	}
	...
	private function getCacheKeyBase () {
		return 'Taggable' . $this->getOwner()->tableName() . $this->tagTable . $this->tagBindingTable . $this->tagTableName . $this->getModelTableFkName() . $this->tagBindingTableTagId . json_encode($this->scope);
	}
	...
	public function getAllTags ( $criteria = null ) {
		$md5 = md5(serialize($criteria));
		//var_dump(implode(',', $criteria->toArray()));

		if ( !($tags = $this->cache->get('Taggable' . $this->getOwner()->tableName() . 'All' . $md5)) ) {
			...
			$findCriteria->join = 'LEFT JOIN ' . $this->tagBindingTable . ' tb ON ( t.id = tb.tagId )';
			$findCriteria->addCondition($this->tagBindingTableModelName . ' = :modelName');
			$findCriteria->params = array(
				':modelName' => $this->modelTableName
			);
			if ( $criteria ) {
				$findCriteria->mergeWith($criteria);
			...
			$this->cache->set('Taggable' . $this->getOwner()->tableName() . 'All' . $md5, $tags);
		}
	...
	public function getAllTagsWithModelsCount ( $criteria = null ) {
		...
		if ( !($tags = $this->cache->get('Taggable' . $this->getOwner()->tableName() . 'AllWithCount' . $md5)) ) {
			// getting associated tags
			$builder = $this->getConnection()->getCommandBuilder();
			...
			$tags = $builder->createFindCommand($this->tagTable, $findCriteria)->queryAll();

			$this->cache->set('Taggable' . $this->getOwner()->tableName() . 'AllWithCount' . $md5, $tags);
		}
