...
class EEavBehavior extends CActiveRecordBehavior {
	...
	public function attach ( $owner ) {
		...
		if ( !is_string($this->tableName) || empty($this->tableName) ) {
			throw new CException(self::t('yii',
	...
	public function loadEavAttributes ( $attributes ) {
		...
		foreach ( $data as $row ) {
			...
			if ( !is_null($current = $this->attributes->itemAt($attribute)) && $current != $value ) {
				$value = is_array($current) ? $current[] = $value : array(
					$current,
					$value
				);
			}
	...
	protected function getLoadEavAttributesCommand ( $attributes ) {
		return $this->getOwner()->getCommandBuilder()->createFindCommand($this->tableName,
				$this->getLoadEavAttributesCriteria($attributes));
	}
	...
	protected function getDeleteCommand ( $attributes = array() ) {
		return $this->getOwner()->getCommandBuilder()->createDeleteCommand($this->tableName,
				$this->getDeleteEavAttributesCriteria($attributes));
	}
