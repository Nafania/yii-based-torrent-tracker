...
<image>
	...
	foreach ( $models AS $model ) {
		...
		<description>
			<?php
			echo CHtml::encode(CHtml::image($model->torrentGroup->getImageUrl(100, 0, true),
					$model->getTitle(),
					array('style' => 'width:100px'))) . '&lt;br&gt;';
			...
			foreach ( $model->getEavAttributesWithKeys() AS $name => $value ) {
				echo '&lt;strong&gt;' . CHtml::encode($name) . '&lt;/strong&gt;: ' . CHtml::encode($value) . '&lt;br&gt;';
			}
			?>
		</description>
		<!--<author>{MANAGING_EDITOR} ({torrent_item.AUTHOR})</author>-->
		<category domain="<?php echo Yii::app()->createAbsoluteUrl($catUrl[0],
			array_splice($catUrl, 1)); ?>"><?php echo $model->torrentGroup->category->getTitle(); ?></category>
