...
class TimeHelper {
	 ...
	 */
	static function timeAgoInWords ( $dateTime, $options = array() ) {
		$now = time();

		if ( (int) $dateTime == $dateTime ) {
			$inSeconds = $dateTime;
		}
		else {
		...
		if ( $diff > abs($now - strtotime($end)) ) {
			...
			elseif ( $years > 0 ) {
				...
				$relativeDate .= $months > 0 ? ($relativeDate ? ', ' : '') . Yii::t('time',
					'{n} month|{n} months',
					$months) : '';
				$relativeDate .= $weeks > 0 ? ($relativeDate ? ', ' : '') . Yii::t('time',
					'{n} week|{n} weeks',
					$weeks) : '';
				$relativeDate .= $days > 0 ? ($relativeDate ? ', ' : '') . Yii::t('time',
					'{n} day|{n} days',
					$days) : '';
			}
			elseif ( abs($months) > 0 ) {
			...
			elseif ( abs($days) > 0 ) {
				...
				$relativeDate .= $hours > 0 ? ($relativeDate ? ', ' : '') . Yii::t('time',
					'{n} hour|{n} hours',
					$hours) : '';
			}
			elseif ( abs($hours) > 0 ) {
				...
				$relativeDate .= $minutes > 0 ? ($relativeDate ? ', ' : '') . Yii::t('time',
					'{n} minute|{n} minutes',
					$minutes) : '';
			}
