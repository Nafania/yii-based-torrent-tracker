...
class ARedisList extends ARedisIterableEntity {
	...
	public function add($item) {
		...
		if (!$this->getConnection()->rpush($this->name,$item)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function remove($item) {
		...
		if (!$this->getConnection()->lrem($this->name,$item,1)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function unshift($item) {
		...
		if (!$this->getConnection()->lpush($this->name,$item)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function shift() {
		...
		$item = $this->getConnection()->lpop($this->name);
		$this->_data = null;
		$this->_count = null;
		return $item;
	}
	...
	public function pop() {
		...
		$item = $this->getConnection()->rpop($this->name);
		$this->_data = null;
		$this->_count = null;
		return $item;
	}
	...
	public function range($start = 0, $stop = -1) {
		...
		return $this->getConnection()->lrange($this->name, $start, $stop);
	}
	/**
	...
	public function trim($start, $stop) {
		...
		return $this->getConnection()->ltrim($this->name, $start, $stop) ? true : false;
	}
	...
	public function getCount() {
		if ($this->_count === null) {
			...
			$this->_count = (int) $this->getConnection()->lSize($this->name);
		}
		return $this->_count;
	}
	/**
