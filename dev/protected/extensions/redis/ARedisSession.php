...
class ARedisSession extends CHttpSession
{
    ...
    /**
     * Идентификатор подключения Redis
     * @var string
     */
	...
	{
		$data=$this->_connection->get($this->calculateKey($id));
	...
	public function writeSession($id, $data)
	{
		...
		$this->_connection->set($key,$data);
		$this->_connection->expire($key,$this->getTimeout());
		return true;
	}
	...
	{
		$this->_connection->delete($this->calculateKey($id));
		return true;
	}
	...
	public function getConnection()
	{
		if ($this->_connection === null) {
			if (!isset(Yii::app()->{$this->connectionID})) {
                throw new CException(get_class($this)." expects a '{$this->connectionID}' application component");
            }
