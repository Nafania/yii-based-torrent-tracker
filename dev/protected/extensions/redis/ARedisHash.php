...
class ARedisHash extends ARedisIterableEntity {
	...
	public function add($key, $value) {
		...
		if (!$this->getConnection()->hset($this->name,$key, $value)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function remove($key) {
		...
		if (!$this->getConnection()->hdel($this->name,$key)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function getCount() {
		if ($this->_count === null) {
			...
			$this->_count = $this->getConnection()->hlen($this->name);
		}
		return $this->_count;
	}
	/**
	...
	public function getData($forceRefresh = false) {
		if ($forceRefresh || $this->_data === null) {
			...
			$this->_data = $this->getConnection()->hgetall($this->name);
		}
		return $this->_data;
	}
