...
class ARedisSortedSet extends ARedisIterableEntity {
	...
	public function add($key, $value) {
		if (!$this->getConnection()->zadd($this->name,$value, $key)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function remove($key) {
		if (!$this->getConnection()->zrem($this->name,$key)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function getCount() {
		if ($this->_count === null) {
			$this->_count = $this->getConnection()->zcard($this->name);
		}
		return $this->_count;
	}
	/**
	...
	public function getData($forceRefresh = false) {
		if ($forceRefresh || $this->_data === null) {
			$this->_data = $this->getConnection()->zrange($this->name,0, -1, true);
		}
		return $this->_data;
	}
