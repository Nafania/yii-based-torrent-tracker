...
class ARedisCounter extends ARedisEntity {
	...
	public function clear() {
		$this->_value = null;
		$this->getConnection()->delete($this->name);
		return $this;
	}
	...
	public function getValue($forceRefresh = false) {
		if ($this->_value === null || $forceRefresh) {
			...
			$this->_value = (int) $this->getConnection()->get($this->name);
		}
		return $this->_value;
	}
	/**
	...
	public function increment($byAmount = 1) {
		...
		return $this->_value = (int) $this->getConnection()->incrBy($this->name,$byAmount);
	}
	...
	public function decrement($byAmount = 1) {
		...
		return $this->_value = (int) $this->getConnection()->decrBy($this->name,$byAmount);
	}
