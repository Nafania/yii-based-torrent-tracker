...
class ARedisSet extends ARedisIterableEntity {
	...
	public function add($item) {
		if (!$this->getConnection()->sadd($this->name,$item)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function remove($item) {
		if (!$this->getConnection()->srem($this->name,$item)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	 ...
	 */
	public function pop() {
		$member = $this->getConnection()->spop($this->name);
		$this->_data = null;
		$this->_count = null;
		return $member;
	}
	...
	public function random() {
		return $this->getConnection()->srandmember($this->name);
	}
	...
	public function move($destination, $item) {
		...
		$this->_count = null;
		$this->_data = null;
		return $this->getConnection()->smove($this->name, $destination, $item);
	}
	...
	public function getCount() {
		if ($this->_count === null) {
			$this->_count = $this->getConnection()->scard($this->name);
		}
		return $this->_count;
	}
	/**
	...
	public function getData($forceRefresh = false) {
		if ($forceRefresh || $this->_data === null) {
			$this->_data = $this->getConnection()->smembers($this->name);
		}
		return $this->_data;
	}
	...
	public function contains($item) {
		return $this->getConnection()->sismember($this->name, $item);
	}
	/**
